...
public class ComputerPlayer : MonoBehaviour
{
    ...
    public PlanOfAttack Evaluate()
    {
        ...
        if (pattern && pattern.pickers.Count > 0)
            pattern.Pick(poa);
        else
    ...
    {
        AbilityRange range = poa.ability.GetComponent<AbilityRange>();
        return range.directionOriented == false;
    }
    ...
    bool IsAbilityTargetMatch(PlanOfAttack poa, Tile tile)
    {
        ...
        else if (poa.target != Targets.None)
        {
            ...
            Stats s = tile.content.GetComponentInChildren<Stats>();
            if (other != null && alliance.IsMatch(other, poa.target) && s != null && s[StatsTypes.HP] > 0)
                isMatch = true;
        }
    ...
    void PickBestOption(PlanOfAttack poa, List<AttackOption> list)
    {
        ...
        {
            poa.ability = null; // Clear ability as a sign not to perform
            return;
        }
