...
public class TestLevelGrowth : MonoBehaviour
{
    void OnEnable()
    {
        this.AddObserver(OnLevelChange, Stats.DidChangeNotifications(StatsTypes.LVL));
        this.AddObserver(OnExperienceException, Stats.WillChangeNotifications(StatsTypes.EXP));
    }

    void OnDisable()
    {
        this.RemoveObserver(OnLevelChange, Stats.DidChangeNotifications(StatsTypes.LVL));
        this.RemoveObserver(OnExperienceException, Stats.WillChangeNotifications(StatsTypes.EXP));
    }
    ...
    void VerifyLevelToExperienceCalculations()
    {
        for (int i = 1; i < 100; ++i)
        {
            ...
            if (lvlExp != i)
                Debug.Log(string.Format("Mismatch on level: {0} with exp: {1} return: {2}", i, expLvl, lvlExp));
            else
    ...
    {
        string[] names = new string[] { "Barret", "Tifa", "Cloud", "Ramza", "Delita", "Orlandeu" };
    ...
    void OnExperienceException(object sender, object args)
    {
        ...
        switch (roll)
        {
            ...
            case 2:
                ...
                break;
            default:
                Debug.Log(string.Format("{0} received {1} experience", actor.name, vce.delta));
                break;
        }
    }
}
